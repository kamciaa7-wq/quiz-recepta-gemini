...
type Stage = 'home'|'form'|'quiz'|'done';
...
export default function Home(){
  ...
  const [rankPlace,setRankPlace] = useState<number | null>(null);
  ...
  async function finish(){
    ...
    const rankingResponse = await fetch('/api/admin/results?public=1');
    const rankingJson = await rankingResponse.json();
    const rows = rankingJson.results || [];
 
    const place = rows.findIndex((r:any) => r.id === attempt);
 
    setRankPlace(place >= 0 ? place + 1 : null);
    ...
    if(stage === 'home') return (
      <>
        ...
        <div className="infoCard">
          <div>📝 {questions.length} pytań</div>
    ...
    if(stage === 'quiz') return (
      <>
        ...
        <div className="quizTop">
          <span>Pytanie {idx + 1}/{questions.length}</span>
          <span>⏱️</span>
        </div>
    ...
    return (
      <>
        ...
        <p className="sub">
          ...
          <b>{result.score} z {questions.length} pytań</b>
        </p>
        ...
        <div className="scoreBox">
          {rankPlace
            ? <>Jesteś aktualnie na miejscu <b>{rankPlace}</b>{rankTotal ? <> z {rankTotal}</> : null}</>
            : <>Wynik zapisany. Ranking zostanie przeliczony.</>
          }
        </div>
  ...
  },[stage,form,idx,answers,err,result,rankPlace,rankTotal]);
